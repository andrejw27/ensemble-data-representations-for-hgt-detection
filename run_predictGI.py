...
def evaluate():
    ...
    genome_folder = genome_folder.split('/')[-1]
    eval_folder = os.path.join(predictions_folder, genome_folder)
    prev_studies_folder = os.path.join(predictions_folder, "prev_studies")
    model_dict = {}
    ...
    for file in os.listdir(eval_folder):
        if file.endswith('.xlsx'):
            ...
            predictor_df = pd.read_excel(os.path.join(eval_folder,file))
            predictor_df.columns = predictor_df.columns.str.capitalize()
            model_dict.update({predictor:get_organism_info(predictor_df)}) 

    #read predictions from other baselines
    for file in os.listdir(prev_studies_folder):
        if file.endswith('.xlsx'):
            predictor = file.split('.xlsx')[0]
            predictor_df = pd.read_excel(os.path.join(prev_studies_folder,file))
            predictor_df.columns = predictor_df.columns.str.capitalize()
            model_dict.update({predictor:get_organism_info(predictor_df)}) 
    ...
    if result_type == 'literature':
        ...
        pos_table = pd.read_excel("outputs/literature_reference/benbow_pos_table.xlsx")
        neg_table = pd.read_excel("outputs/literature_reference/benbow_neg_table.xlsx")
    ...
    json_file = f"{eval_dest}/evaluation_result_{result_type}_gridsearch_default.json"
...
def main():
    ...
    if result_type == "test":
        genome_folder = "dataset/genomes/benbow_set"
    elif result_type == "literature":
        genome_folder = "dataset/genomes/literature_set"
    ...
    models = ['ensemble_stacking_benbow_default.pkl',
              ...
              'single_benbow_fine_tuned.pkl',
              'ensemble_voting_soft_benbow_fine_tuned_default.pkl',
              'ensemble_stacking_benbow_fine_tuned_default.pkl']
